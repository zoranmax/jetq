...
from typing import (
    Iterable, Iterator, TypeVar, Generic, Callable, Optional, 
    Dict, List, Tuple, Any, Set, Union
)
...
class OrderedQueryable(Generic[T]):
    ...
    def __init__(self, source: 'Queryable[T]', orderings: List[Tuple[KeySelector, bool]]):
        self.source = source
    ...
    def then_by_descending(self, key_selector: KeySelector[T, Any]) -> 'OrderedQueryable[T]':
    ...
    def where(self, predicate: Predicate[T]) -> 'Queryable[T]':
        """Filter elements."""
        return Queryable(self).__where(predicate)
...
class Queryable(Generic[T]):
    ...
    def distinct(self, key_selector: Optional[KeySelector[T, Any]] = None) -> 'Queryable[T]':
        """Remove duplicate elements.
        
        Args:
            key_selector: Optional function to determine uniqueness. If None, uses element equality.
    ...
    def order_by_descending(self, key_selector: KeySelector[T, Any]) -> OrderedQueryable[T]:
    ...
    def group_by(self, key_selector: KeySelector[T, TKey]) -> 'Queryable[GroupingResult[TKey, T]]':
    ...
    def join(self, 
             ...
             result_selector: Callable[[T, TInner], TResult]) -> 'Queryable[TResult]':
    ...
    def group_join(self,
                   ...
                   outer_key_selector: KeySelector[T, Any],
                   inner_key_selector: KeySelector[TInner, Any],
                   result_selector: Callable[[T, List[TInner]], TResult]) -> 'Queryable[TResult]':
    ...
    def average(self, selector: Optional[Selector[T, Union[int, float]]] = None) -> float:
        ...
        if selector is None:
            ...
            total = sum(selector(item) for item in items)
    ...
    def aggregate(self, 
                  func: Aggregator[TResult, T],
                  seed: Optional[TResult] = None) -> TResult:
    ...
    def last(self, predicate: Optional[Predicate[T]] = None) -> T:
        ...
        if not found:
            raise IndexError("Sequence is empty" if predicate is None else "No matching element found")
    ...
    def single(self, predicate: Optional[Predicate[T]] = None) -> T:
        ...
        if count == 0:
            raise ValueError("Sequence is empty" if predicate is None else "No matching element found")
    ...
    def contains(self, value: T, key_selector: Optional[KeySelector[T, Any]] = None) -> bool:
    ...
    def to_dict_by_key_value(self,
                            key_selector: KeySelector[T, TKey],
                            value_selector: Selector[T, TValue]) -> Dict[TKey, TValue]:
