...
def large_csv_file(tmp_path):
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["id", "name", "email", "value", "category"]
        )
        writer.writeheader()
...
def test_select_10_items_from_row_400(large_csv_file):
    ...
    result = Queryable(data).skip(399).take(10).to_list()
...
def test_select_10_items_with_ordering(large_csv_file):
    ...
    ordered = Queryable(data).order_by_descending(lambda x: x["value"]).to_list()
    result = Queryable(ordered).take(10).to_list()
...
def test_skip_and_take_pagination(large_csv_file):
    ...
    result = (
        Queryable(data).skip((page_number - 1) * page_size).take(page_size).to_list()
    )
...
def test_complex_query_on_large_dataset(large_csv_file):
    ...
    # Find rows with value > 4000, order by id desc,
    # skip 5, take 10, project id and value
    ordered = (
        Queryable(data)
...
def test_aggregation_on_large_dataset(large_csv_file):
    ...
    high_value_count = Queryable(data).where(lambda x: x["value"] > 500000).count()
...
def test_performance_skip_take_from_middle(large_csv_file):
    ...
    result = Queryable(data).skip(4999).take(10).to_list()
...
def test_distinct_on_large_dataset(large_csv_file):
    ...
    categories = Queryable(data).select(lambda x: x["category"]).distinct().to_list()
